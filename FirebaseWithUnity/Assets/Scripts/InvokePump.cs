...
public class InvokePump : WaitHandle
{
    ...
    {
        _threadId = (ownerThread != null) ? ownerThread.ManagedThreadId : CurrentThread.ManagedThreadId;
    ...
    {
        get { return _threadId != 0 && CurrentThread.ManagedThreadId != _threadId; }
    }
    ...
    public void Invoke(Action action)
    {
        ...
        if (CurrentThread.ManagedThreadId == _threadId) throw new InvalidOperationException("Never call WaitOne on an InvokePump from the thread that owns it, this will freeze that thread indefinitely.");
    ...
    {
        if (_threadId == 0) throw new System.InvalidOperationException("InvokePump has been closed.");
        if (CurrentThread.ManagedThreadId == _threadId) throw new System.InvalidOperationException("Never call WaitOne on an InvokePump from the thread that owns it, this will freeze that thread indefinitely.");
